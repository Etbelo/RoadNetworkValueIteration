...
/**
 * @brief Collection holding all elements of the decoded state.
 *
 ...
 * iteration algorithm.
 *
 * @param[in] P Reference to probability matrix (Eigen csr)
 ...
 * @param[in] P Reference to P matrix (Eigen csr)
...
auto UpdateBlock(
    ...
    int max_actions, float alpha, int num_nodes, int num_nodes_sq) -> float;
 ...
 * @param[in] num_nodes Total number of nodes in mdp
 * @param[in] num_nodes_sq Total number of nodes squared in mdp
 * @return StateTuple Decoded state (cur_charge, tar_node, cur_node)
 */
auto get_state_tuple(int state, int num_nodes, int num_nodes_sq) -> StateTuple;

/**
 * @brief Get stage cost given the state and taken action.
 *
 * @param[in] state_tuple Tuple of decoded state
 * @param[in] action Taken action
 * @return float Stage cost
 */
auto get_stage_cost(StateTuple state_tuple, int action) -> float;
